...
# Here we will just define a few helper functions for building embeding matrix
...
def build_rnn_network(emb_matrix_path, input_length):
    """This function builds RNN network to solve the prediction problem.
...
model = build_rnn_network('fasttext-300.npy', padded_sequences.shape[1])
...
model = build_rnn_network('word2vec-300.npy', padded_sequences.shape[1])
...
model = build_rnn_network('glove-300.npy', padded_sequences.shape[1])
validation_split = 0.2
epochs = 50
batch_size = 100
history = model.fit(padded_sequences, data['Price'], validation_split=validation_split,  
